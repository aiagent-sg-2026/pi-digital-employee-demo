...
async function renderInbox(){
  ...
  if(isHome){
    inboxList.innerHTML=visible.map(item=>{const present=presentInboxIssue(item);return`<li class="inbox-preview-row"><div class="inbox-copy"><strong>${esc(present.title)}</strong><small>${esc(inboxTypeLabel(item.type))} · ${esc(severityLabel(item.severity))} · ${esc(inboxStatusLabel(item.status))}</small></div></li>`}).join("")+`<li class="inbox-preview-more"><a class="link-btn" href="${routeHref("/inbox")}">${esc(t("common.viewAll"))}</a></li>`;return;
  }
  const customers=await business.listCustomers();
  ...
  const entityLabel=async(item:BusinessInboxItem)=>{
    ...
    if(item.relatedEntityType==="approval"){const approval=approvals.find(value=>value.id===item.relatedEntityId);return approval?approvalPresentation(approval).title:item.relatedEntityId;}
...
async function showCustomer(id:string){
  ...
  customerDetail.innerHTML=`<h3>${esc(customer.name)}</h3><p>${esc(customer.code)} · ${esc(t(`common.status.${customer.status}`))} · ${esc(t("common.risk"))} ${esc(riskLabel(customer.risk))}</p>
    ...
    <div class="customer-subsection"><div class="subsection-head"><h4>${esc(t("customers.exceptions"))}</h4><a class="link-btn" href="${routeHref("/inbox",{customerId:customer.id})}">${esc(t("customers.viewExceptions"))}</a></div>${relatedIssues.length?`<div class="customer-exception-list">${relatedIssues.map(item=>{const present=presentInboxIssue(item);return`<article><span class="issue-chip severity ${esc(item.severity)}">${esc(severityLabel(item.severity))}</span><div><strong>${esc(present.title)}</strong><small>${esc(present.detail)}</small><small>${esc(inboxTypeLabel(item.type))} · ${esc(inboxStatusLabel(item.status))}</small></div></article>`}).join("")}</div>`:`<p class="approval-empty">${esc(t("customers.noExceptions"))}</p>`}</div>
...
function renderSummary(summary:Record<string,unknown>){
  ...
  if(summary.resultType==="investigation-completed"){const count=Number(summary.count??0);const items=((summary.paymentExceptions??summary.exceptions) as any[]??[]);return`<div class="result-notice attention"><strong>${esc(t("task.investigationCompleted"))}</strong><p>${esc(count?t("task.exceptionsFound",{count}):t("task.noFollowup"))}</p></div>${items.map(item=>{const present=presentInboxIssue(item);return`<div class="attention-box"><strong>${esc(present.title)}</strong><br>${esc(present.detail)}</div>`}).join("")}`;}
...
async function renderTaskPanels(task:BusinessTask,events:any[],evidence:any[],relatedIssues:BusinessInboxItem[]){
  ...
  taskVerificationPanel.innerHTML=`<h3>${esc(t("verification.title"))}</h3><p><strong>${esc(verificationText)}</strong></p><ul class="check-list">${verificationEvidence?.checks?.map((check:any)=>verificationCheckHtml(check,task)).join("")??""}</ul>`;
  ...
  taskIssuesPanel.innerHTML=`<h3>${esc(t("task.tab.issues"))}</h3>${relatedIssues.length?relatedIssues.map(item=>{const present=presentInboxIssue(item);return`<article class="related-issue"><strong>${esc(present.title)}</strong><span>${esc(present.detail)}</span><span>${esc(inboxTypeLabel(item.type))} · ${esc(severityLabel(item.severity))} · ${esc(inboxStatusLabel(item.status))}</span>${present.resolution?`<span>${esc(present.resolution)}</span>`:""}</article>`}).join(""):`<p class="approval-empty">${esc(t("task.noIssues"))}</p>`}`;
  const approval=approvals.find(value=>value.taskId===task.id);
  taskApprovalPanel.innerHTML=approval?`<h3>${esc(t("task.relatedApproval"))}</h3><p><a class="link-btn" href="${routeHref(`/approvals/${encodeURIComponent(approval.id)}`)}">${esc(approvalPresentation(approval).title)}</a></p><p>${esc(t("connections.status"))}: <strong>${esc(t(`approvals.state.${approval.state}`))}</strong></p>`:`<h3>${esc(t("task.relatedApproval"))}</h3><p class="approval-empty">${esc(t("task.noApproval"))}</p>`;
}
...
async function renderTaskDetail(taskId:string){
  ...
  if(relatedIssues.length)resultHtml+=`<div class="related-issues"><h4>${esc(t("task.relatedInbox"))}</h4>${relatedIssues.map(item=>{const present=presentInboxIssue(item);return`<div class="related-issue"><strong>${esc(present.title)}</strong><span>${esc(severityLabel(item.severity))} · ${esc(inboxStatusLabel(item.status))}</span></div>`}).join("")}</div>`;
  ...
  verificationList.innerHTML=verificationEvidence?.checks?.length?verificationEvidence.checks.map((check:any)=>verificationCheckHtml(check,task)).join(""):`<li><span class="check-icon">${svgIcon("circle")}</span><div>${esc(t("verification.waiting"))}</div></li>`;
...
async function actOnInbox(id:string,action:string){
  ...
  else if(action==="investigate")await work.updateInboxStatus(id,"investigating","Investigation opened in Local Demo Simulation.",{key:"issue.resolution.investigationOpened"});
  await renderAll();
