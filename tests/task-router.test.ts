...
describe("Business World task router",()=>{
  ...
  it("routes customer lookup and brief",()=>{expect(routeDashboardTask("Resolve ambiguous customer.")).toMatchObject({intent:"receivables.review",customerQuery:"Twin"});expect(routeDashboardTask("Prepare today's brief.").intent).toBe("daily.brief");});
